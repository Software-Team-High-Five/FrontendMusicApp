<template>
    <div v-if="loaded">
                    ...
                    >
                        ...
                        <template v-slot:[`item.actions`]="{ item: song }">
                            <v-icon
                                small
                                class="mr-2"
                                @click="openSongDialog(song, false)"
                            >
                                mdi-pencil
                            </v-icon>
                            <v-icon
                        ...
                        <template v-slot:top>
                            <v-toolbar flat>
                                <v-toolbar-title class="body-1"><strong>Repertoire</strong></v-toolbar-title>
                                <v-spacer></v-spacer>
                                <v-btn @click="openSongDialog({}, true)" >
                                    Add
                                </v-btn>
                            </v-toolbar>
                        </template>
        ...
        >
            <v-card>
                <v-card-title>Are you sure you want to delete "{{ this.selectedSong.title }}"?</v-card-title>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn @click="deleteSong">
                        Delete
                    </v-btn>
...
export default {
    ...
    ,methods: {
        ...
        editSong() {
            songDS.update(this.selectedSong.id, {
                title: this.selectedSong.title
                ,translation: this.selectedSong.translation
                ,composerId: this.selectedSong.composerId
                ,instrumentId: this.selectedSong.instrumentId
        ...
        deleteSong() {
            songDS.delete(this.selectedSong.id)
                .then(() => {
    ...
    ,mounted() {
        // Get the instructor
        let instructorPromise = userDS.get(this.userStore.user.student.instructorId)
            .then(res => {
        ...
        let songsPromise = songDS.getAll()
            .then((res) => {
        ...
        // Get the composers
        let composersPromise = composerDS.getAll()
            .then((res) => {
                this.allComposers = res.data.map(composer => {
                    return {
                        id: composer.id
                        ,name: [composer.fName, composer.mName, composer.lName].filter(Boolean).join(' ')
                    };
                });
            })
            .catch((e) => {
